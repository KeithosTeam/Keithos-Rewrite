...
module.exports = class catfact extends Command {
	...
	async run(message, args) {
        ...
        if (channel.type != 'text' || !channel.viewable) return this.utils.sendErrorMessage(message, 0, stripIndent`
        ...
        if (modChannelIds.includes(channel.id)) return this.utils.sendErrorMessage(message, 0, stripIndent`
          Provided channel is moderator only, please mention an accessible text channel or provide a valid text channel ID
        `);
    
        if (!args[0]) return this.utils.sendErrorMessage(message, 0, 'Please provide a message for me to say');
    
        // Check channel permissions
        if (!channel.permissionsFor(message.guild.me).has(['SEND_MESSAGES']))
          return this.utils.sendErrorMessage(message, 0, 'I do not have permission to send messages in the provided channel');
    
        if (!channel.permissionsFor(message.member).has(['SEND_MESSAGES']))
          return this.utils.sendErrorMessage(message, 0, 'You do not have permission to send messages in the provided channel');
    
        const msg = message.content.slice(message.content.indexOf(args[0]), message.content.length);
        message.channel.send({ embeds: [embed] }, { disableMentions: 'everyone' });
	}
};
