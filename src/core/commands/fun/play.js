...
module.exports = class music extends Command {
    ...
    const x = message.content.replace(prefix, "").split(" ")[0]
      ...
      if (!voice_channel) return message.channel.send({ content: 'You need to be in a channel to execute this command!' });
...
const video_player = async (guild, song) => {
  ...
  const skip_song = (message, server_queue) => {
    if (!message.member.voice.channel) return message.channel.send({ content: 'You need to be in a channel to execute this command!' });
    if(!server_queue){
