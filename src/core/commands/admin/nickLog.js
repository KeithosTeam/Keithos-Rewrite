...
module.exports = class joinLog extends Command {
	...
	async run (message, args) {
		...
		Schema.findOne({ _id: message.guild.id }, async (e, data) => {
			...
			if (!data) {
				data = new Schema({ _id: message.guild.id, nickLog: channel.id});
			} else {
				return data.updateOne({ _id: message.guild.id, nickLog: channel.id });
			}

			data.save();
		});

		message.channel.send({ content: this.emoji.tick + ` ${channel} is set as nick log`});
	}
};
